...
def get_stg_data(debug=True):
    query = """
        declare @src varchar(99) = '[DataStage].[STG_ShiptMasterSales]'
        ...
        ,sum(cast(sales.Dollars as money)) [Dollars]
        --,count (*) [Rows]
        FROM [TDWH].[DataStage].STG_ShiptMasterSales sales
...
def get_ecommsldw_data(debug=True):
    query = """
        declare @src varchar(99) = '[landing].[Shipt]'
        ...
        ,sum (sale.Dollars) [Dollars]
        --,count (*) [Rows]
        FROM [ECommSalesDW].[landing].[Shipt] sale
