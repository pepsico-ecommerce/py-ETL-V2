...
def get_ecommsmeas_data(debug=True):
    query = """
        declare @src varchar(99) = '[measure].[vw_productSaleDaily]'
        ...
        ,dte.fiscalPeriod
        ,dte.fiscalPeriodWeekNumber
        ,sum (sale.[OrigRSV]) [Dollars]
        --,count (*) [Rows]
        FROM [ECommSalesDW].[measure].[vw_productSaleDaily] sale
        join ECommSalesDW.dim.date dte on dte.dateId =  sale.[dayOfSale]
        join ECommSalesDW.dim.customer cust on cust.[customerId] =  sale.[customerId]
        where dte.calendarWeekStartDate >= dateadd(dd, -60, getdate())
		and cust.customerName = 'albertsons'
        group by
        dte.calendarWeekStartDate
        ,dte.fiscalYear
        ,dte.fiscalYearWeekNumber
...
if __name__ == "__main__":
    ...
    df_esdw = get_ecommsmeas_data(debug=debug)
    if (debug):
    ...
    df_final.insert(1, 'Measure_Src', df_esdw['src'])
    df_final['Measure_Dollars'] = df_esdw['Dollars']
    df_final['Diff'] = np.where(df_final['Stg_Dollars'] == df_final['Measure_Dollars'], 0, df_final['Measure_Dollars'] - df_final['Stg_Dollars'])
